...
namespace CNFL_Clientes_Prototipo.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        {
            // Evitar llamadas no traducibles a SQL dentro de la proyección (ToString/formateo)
            var totalAverias = _db.Averias.Count();
            var averiasPorMesRaw = _db.Averias
                .GroupBy(a => a.FechaReporte.Month)
                .Select(g => new { Mes = g.Key, Cantidad = g.Count() })
                .ToList();

            var averiasRecientesEntities = _db.Averias
            ...
            {
                TotalAverias = totalAverias,
                TiempoPromedioResolucion = "2.5 hrs",
                TasaResolucion = 85,
                AveriasPorMes = averiasPorMesRaw
                    .Select(x => new AveriaPorMes { Mes = x.Mes.ToString(), Cantidad = x.Cantidad })
                    .ToList(),
                AveriasRecientes = averiasRecientesEntities
                    .Select(a => new AveriaConTiempo
                    {
                        AveriaId = a.AveriaId,
                        ...
                        Estado = a.Estado,
                        TiempoTranscurrido = (DateTime.Now - a.FechaReporte).ToString(@"hh\:mm")
                    }).ToList()
            };
        ...
        // POST: Admin/ActualizarEstadoAveria
        [HttpPost]
        ...
        public ActionResult ReportesPDF()
        {
            var averias = _db.Averias
                .Include("Usuario")
                .Include("NISE")
            ...
            // Aquí puedes generar un PDF con iTextSharp o la librería que uses
            return View(averias);
        }
