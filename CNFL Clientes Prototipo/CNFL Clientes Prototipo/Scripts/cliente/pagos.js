...
function mostrarProcesamiento(metodo, monto, facturaId) {
    ...
    overlay.style.cssText = `
        ...
        animation: fadeInPago 0.4s ease;
    `;
    ...
    overlay.innerHTML = `
        <div style="text-align: center; max-width: 340px; width: 100%; padding: 20px;">
            ...
            <div style="font-size: 14px; color: rgba(255,255,255,0.6); margin-bottom: 4px;">${nombresMetodo[metodo] || metodo}</div>
            
            <div style="font-size: 28px; font-weight: 800; color: ${coloresMetodo[metodo] || '#64B95A'}; margin: 12px 0 20px;">
            ...
            <div style="width: 100%; height: 6px; background: rgba(255,255,255,0.1); border-radius: 6px; overflow: hidden; margin-bottom: 16px;">
                <div id="barraProgresoPago" style="width: 0%; height: 100%; background: linear-gradient(90deg, ${coloresMetodo[metodo] || '#1E23E6'}, ${metodo === 'Sinpe' ? '#43A047' : metodo === 'Iban' ? '#5a60f5' : '#ff8a5c'}); border-radius: 6px; transition: width 0.8s ease;"></div>
            </div>
            
            <div id="estadoProcesamiento" style="font-size: 14px; color: rgba(255,255,255,0.7); font-weight: 500;">
                Iniciando transacción...
            </div>
            
            <div style="margin-top: 20px; display: flex; flex-direction: column; gap: 6px; text-align: left; padding: 0 8px;">
                <div id="paso1" style="display: flex; align-items: center; gap: 10px; font-size: 12px; color: rgba(255,255,255,0.3);">
                    <span style="font-size: 14px;">○</span>
                    <span>Conectando con el banco</span>
                </div>
                <div id="paso2" style="display: flex; align-items: center; gap: 10px; font-size: 12px; color: rgba(255,255,255,0.3);">
                    <span style="font-size: 14px;">○</span>
                    <span>Verificando datos</span>
                </div>
                <div id="paso3" style="display: flex; align-items: center; gap: 10px; font-size: 12px; color: rgba(255,255,255,0.3);">
                    <span style="font-size: 14px;">○</span>
                    <span>Autorizando transacción</span>
                </div>
                <div id="paso4" style="display: flex; align-items: center; gap: 10px; font-size: 12px; color: rgba(255,255,255,0.3);">
                    <span style="font-size: 14px;">○</span>
        ...
        overlay.innerHTML = `
            <div style="text-align: center; max-width: 380px; width: 100%; padding: 20px;">
                <div style="width: 100px; height: 100px; border-radius: 50%; background: rgba(100, 185, 90, 0.15); display: flex; justify-content: center; align-items: center; margin: 0 auto 20px; border: 3px solid #64B95A; animation: checkBouncePago 0.6s ease;">
                    <svg viewBox="0 0 24 24" width="56" height="56" fill="none" stroke="#64B95A" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div style="font-size: 30px; font-weight: 900; color: #64B95A; margin-bottom: 16px;">
                    ₡ ${parseInt(monto).toLocaleString()}
                </div>
                ...
                <div style="background: rgba(255,255,255,0.06); border-radius: 16px; padding: 16px 20px; border: 1px solid rgba(255,255,255,0.08); margin-bottom: 16px; text-align: left;">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                        <div>
                            <div style="font-size: 10px; color: rgba(255,255,255,0.4);">Transacción</div>
                            <div style="font-size: 13px; font-weight: 700; color: white;">${transaccion}</div>
                        </div>
                        <div>
                            <div style="font-size: 10px; color: rgba(255,255,255,0.4);">Autorización</div>
                            <div style="font-size: 13px; font-weight: 700; color: white;">${autorizacion}</div>
                        </div>
                        <div>
                            <div style="font-size: 10px; color: rgba(255,255,255,0.4);">Fecha</div>
                            <div style="font-size: 13px; font-weight: 700; color: white;">${new Date().toLocaleString('es-CR')}</div>
                        </div>
                        <div>
                            <div style="font-size: 10px; color: rgba(255,255,255,0.4);">Factura</div>
                            <div style="font-size: 13px; font-weight: 700; color: white;">#${facturaId || 'N/A'}</div>
                        </div>
                    </div>
                </div>
                
                <button onclick="cerrarPago()" style="padding: 14px 32px; border-radius: 14px; border: none; background: linear-gradient(135deg, #1E23E6, #5a60f5); color: white; font-weight: 800; font-size: 14px; cursor: pointer; transition: 0.2s; font-family: inherit; box-shadow: 0 6px 20px rgba(30,35,230,0.3); width: 100%;">
                    <i class="fas fa-file-invoice"></i> Ver comprobante
                </button>
                
                <div style="margin-top: 12px; font-size: 12px; color: rgba(255,255,255,0.3);">
                    Se envió un comprobante a tu correo electrónico
                </div>
            </div>
        `;
    }
        ...
        overlay.innerHTML = `
            <div style="text-align: center; max-width: 340px; width: 100%; padding: 20px;">
                <div style="width: 100px; height: 100px; border-radius: 50%; background: rgba(229, 72, 77, 0.15); display: flex; justify-content: center; align-items: center; margin: 0 auto 20px; border: 3px solid #E5484D;">
                    <svg viewBox="0 0 24 24" width="48" height="48" fill="none" stroke="#E5484D" stroke-width="3">
                ...
                <div style="display: flex; gap: 12px;">
                    <button onclick="cerrarPago()" style="flex: 1; padding: 14px; border-radius: 14px; border: 1.5px solid rgba(255,255,255,0.15); background: transparent; color: rgba(255,255,255,0.7); font-weight: 700; font-size: 14px; cursor: pointer; font-family: inherit;">
                        Cancelar
                    </button>
                    <button onclick="cerrarPago(); setTimeout(function(){ iniciarPago('${metodo}', '${facturaId}'); }, 300);" style="flex: 2; padding: 14px; border-radius: 14px; border: none; background: linear-gradient(135deg, #1E23E6, #5a60f5); color: white; font-weight: 800; font-size: 14px; cursor: pointer; font-family: inherit; box-shadow: 0 6px 20px rgba(30,35,230,0.3);">
...
function iniciarPago(metodo, facturaId) {
    console.log('🔘 iniciarPago llamado - Método:', metodo, 'Factura:', facturaId);
    ...
    console.log('✅ Iniciando pago - Método:', metodo, 'Factura:', facturaId, 'Monto:', monto);
